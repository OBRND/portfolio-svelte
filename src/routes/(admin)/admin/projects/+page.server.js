...
const SCREENSHOTS_BUCKET = "portfolio"

/**
 * Uploads a screenshot to the public `portfolio` bucket and returns its
 * permanent public URL, or `null` if no file was actually chosen (an empty
 * file input still submits as a zero-byte File, not `undefined`).
 *
 * Runs through `locals.supabase` rather than the anon-key `supabaseServer`
 * singleton used elsewhere in this file, so the request authenticates as the
 * signed-in admin — see the storage policies in
 * supabase/migrations/0002_public_screenshots_bucket.sql, which only grant
 * upload/replace/delete to `authenticated`.
 */
async function uploadScreenshot(locals, file, category) {
  if (!file || typeof file === "string" || file.size === 0) return null

  const extMatch = /\.([a-z0-9]+)$/i.exec(file.name || "")
  const ext = extMatch ? extMatch[1].toLowerCase() : "jpg"
  const folder = category === "web" ? "web" : "android"
  ...
  const { data } = locals.supabase.storage.from(SCREENSHOTS_BUCKET).getPublicUrl(path)
...
export const actions = {
  create: async ({ request, locals }) => {
    try {
      ...
      // Which stack the project appears in on the landing page. Anything other
      // than an explicit "web" falls back to the mobile stack.
      const category = formData.get("category")?.toString().trim().toLowerCase() === "web" ? "web" : "flutter"
      ...
      // An uploaded screenshot wins over a manually typed URL — the file
      // input and the URL field both write to the same `image_link` column.
      let uploadedUrl
      try {
        uploadedUrl = await uploadScreenshot(locals, formData.get("screenshot"), category)
      } catch (uploadErr) {
        return fail(500, { error: true, message: uploadErr.message })
      }
      const finalImageLink = uploadedUrl || image_link || null
  ...
  update: async ({ request, locals }) => {
    try {
      ...
      // A newly uploaded screenshot replaces the current image; otherwise the
      // URL field (already pre-filled with the existing image_link by the
      // edit form) carries the current value forward unchanged.
      let uploadedUrl
      try {
        uploadedUrl = await uploadScreenshot(locals, formData.get("screenshot"), category)
      } catch (uploadErr) {
